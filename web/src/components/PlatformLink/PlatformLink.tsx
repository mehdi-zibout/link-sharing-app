...
const PlatformLink = ({ path, platformId }: PlatformLinkProps) => {
  ...
  if (platform.id === 'FRONTEND_MENTOR') {
    return (
      <a
        href={path}
        className="group flex items-center justify-between rounded-lg border border-borders bg-white p-4 text-bm text-dark-grey"
        target="_blank"
        rel="noreferrer"
      >
  ...
  return (
    <a
      href={path}
      className="group flex items-center justify-between rounded-lg p-4 text-bm text-white"
